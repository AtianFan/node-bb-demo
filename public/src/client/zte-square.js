...
define('forum/zte-square', [
	...
	function removeListeners() {
		socket.removeListener('event:new_topic', Square.onNewTopic);
	...
	Square.init = function () {
		...
		socket.removeListener('event:new_topic', Square.onNewTopic);
		socket.on('event:new_topic', Square.onNewTopic);
		...
		if (!config.usePagination) {
			navigator.init('[component="category/topic"]', ajaxify.data.topic_count, Square.toTop, Square.toBottom, Square.navigatorCallback);
		}
	...
	Square.toTop = function () {
		navigator.scrollTop(0);
	};

	Square.toBottom = function () {
	...
	Square.navigatorCallback = function (topIndex, bottomIndex, elementCount) {
		return bottomIndex;
	};
			...
			if (config.usePagination) {
				...
				if (parseInt(page, 10) !== ajaxify.data.pagination.currentPage) {
					pagination.loadPage(page, function () {
						Square.scrollToTopic(bookmarkIndex, clickedIndex, 400);
					});
				} else {
					Square.scrollToTopic(bookmarkIndex, clickedIndex, 400);
				}
				...
				if (bookmarkIndex === 0) {
					Square.highlightTopic(clickedIndex);
					return;
				}
				...
				loadTopicsAfter(Math.max(0, bookmarkIndex - 1) + 1, 1, function () {
					$(window).one('action:topics.loaded', function () {
						Square.scrollToTopic(bookmarkIndex, clickedIndex, 0);
					});
				});
			}
		}
	...
	Square.highlightTopic = function (topicIndex) {
	...
	Square.scrollToTopic = function (bookmarkIndex, clickedIndex, duration, offset) {
		if (!bookmarkIndex) {
			return;
		}
	...
	function enableInfiniteLoadingOrPagination() {
		if (!config.usePagination) {
			infinitescroll.init($('[component="category"]'), Square.loadMoreTopics);
		} else {
			navigator.disable();
		}
	}

	Square.onNewTopic = function (topic) {
		var	cid = ajaxify.data.cid;
	...
	Square.loadMoreTopics = function (direction) {
	...
	function loadTopicsAfter(after, direction, callback) {
		...
		infinitescroll.loadMore('topics.loadMoreByRootcid', {
			...
			if (data.topics && data.topics.length) {
				Square.onTopicsLoaded(data, direction, done);
			} else {
				done();
			}
	...
	Square.onTopicsLoaded = function (data, direction, callback) {
