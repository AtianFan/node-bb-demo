...
<div class="row zte-category-list">
	<div class="category col-lg-9 col-sm-12" has-widget-class="category zte-category-list col-lg-9 col-sm-12" has-widget-target="sidebar">
		...
		<div class="clearfix">
			<!-- BEGIN tagWhitelistWhole -->
			<div class="tags pull-left">
				<!-- IF tagWhitelistWhole.value -->
				<a href="{config.relative_path}/category/{slug}?tag={tagWhitelistWhole.value}" class="{tagWhitelistWhole.on}">
					<span class="tag-item" data-tag="{tagWhitelistWhole.value}" style="">{tagWhitelistWhole.value}</span>
					<span class="tag-topic-count human-readable-number" title="{tagWhitelistWhole.score}">{tagWhitelistWhole.score}</span>
				</a>
				<!-- ELSE -->
				<a href="{config.relative_path}/category/{slug}" class="{tagWhitelistWhole.on}">
					<span class="tag-item" style="">{tagWhitelistWhole.name}</span>
				</a>
				<!-- ENDIF tagWhitelistWhole.value -->
			</div>
			<!-- END tagWhitelistWhole -->
		</div>
