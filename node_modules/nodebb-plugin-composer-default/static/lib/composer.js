...
define('composer', [
	...
	composer.bignewsPost = function(pid,bignews) {
	...
	composer.enhance = function(postContainer, post_uuid, postData) {
		...
		*/
		...
		var proMore = postContainer.find('.pro-more');
		proMore.on('click', function() {
			$(this).attr('disabled', true);
			socket.emit('topics.readjson', function (err, data){
				if (err) {
					return app.alertError(err.message);
				}
				var picArr = data.pics.RECORDS;
				var topicArr = data.topics.RECORDS.slice(1082,1300);
				var composerDataArr = [];
				topicArr.forEach(function(item,index){
					if(!item.message){
						item.message = '无';
					}else{
						item.message = item.message.replace(/\[color=?.*?\]\s*|\[font=?.*?\]\s*|\[size=?.*?\]\s*|\[backcolor=?.*?\]\s*|\[list\]|\[\/(color|font|size|backcolor)\]/ig,'')
							.replace(/\[i=s\]\s*|\s*\[\/i\]/ig,'*')
							.replace(/\[b\]\s*/ig,'**')
							.replace(/\[\/b\]\s*/ig,'** ')
							.replace(/\[align=center\]\s*/ig,'## ')
							.replace(/\[align=left\]\s*|\[align=right\]\s*/ig,'\n')
							.replace(/\[p=?.*?\]/ig,'')
							.replace(/\[\*\]\s*/ig,'* ')
							.replace(/\[\/list\]/ig,'\n')
							.replace(/\[\/(align|p)\]\s*/ig,'\n')
							.replace(/\[url=?.*?\](.+?)\[\/url\]/ig,function(str){
								var url = str.match(/\[url=?.*?\]/g);
								var re = url[0].replace('[url=',' ').replace(']',' ');
								return re;
							})
							.replace(/\[attach\](.+?)\[\/attach\]/ig,function(str){
								var aid = str.match(/\d+/g);
								var pic = picArr.filter(function(item){return item.aid == aid[0]})
								if(pic.length == 0){
									return str;
								}
								var re = '!['+pic[0].filename+'](/uploads/files/'+pic[0].attachment+')\n'
								return re;
							})
					}
					var tempArr = item.typeName ? [item.typeName] : [];
					composerDataArr.push({
						cid:'67',
				...
				// 	title:'create test'
				// }
				// socket.emit('topics.post', composerData, function (err, data) {
				// 	postContainer.find('.pro-more').removeAttr('disabled');
				// 	if (err) {
				...
				// });
				// var composerDataArr = [];
				socket.emit('topics.batchpost', composerDataArr, function (err, data) {
					postContainer.find('.pro-more').removeAttr('disabled');
					if (err) {
						if (err.message === '[[error:email-not-confirmed]]') {
							return app.showEmailConfirmWarning(err);
		...
		if(breadcrumb.length > 0 && breadcrumb.children('li').length > 2){
			var url = breadcrumb.children('li').eq(1).find('a').attr('href');
			composer.posts[post_uuid].parentCid = [];
			composer.posts[post_uuid].parentCid.push(url.split('/')[2]);
		}else{
			composer.posts[post_uuid].parentCid = [0];
		}
	...
	function createNewComposer(post_uuid) {
		...
		var data = {
			...
			bignews: postData ? postData.bignews : undefined
		};
	...
	function post(post_uuid,target) {
		var postData = composer.posts[post_uuid];
		var postContainer = $('#cmp-uuid-' + post_uuid);
		var handleEl = postContainer.find('.handle');
		var titleEl = postContainer.find('.title');
		var bodyEl = postContainer.find('textarea');
		var previewEl = postContainer.find('.preview-container');
		var previewImgEl = previewEl.find('img:not(.emoji)');
		var categoryEl = postContainer.find('.category-list');
		var thumbEl = postContainer.find('input#topic-thumb-url');
		var trophyEl = $('[component="post/header"] i.fa-trophy');
		...
		//添加头条时检测是否存在图片
		} else if (previewImgEl.length == 0 && postData.isMain && postData.bignews) {
			return composerAlert(post_uuid, '[[error:bignews-no-img]]');
		//编辑时检测是否存在图片
		} else if (previewImgEl.length == 0 && postData.isMain && postData.title != '' && !trophyEl.hasClass('hidden')) {
			return composerAlert(post_uuid, '[[error:bignews-no-img]]');
		}

		var composerData = {};
		var isAnonymous = (target.attr('id') == 'anonymous') ? true : false;
		...
		if (action === 'topics.post') {
			...
			composerData = {
				...
				thumb: thumbEl.val() || '',
				tags: tags.getTags(post_uuid),
				bignews: postData.bignews ? true: undefined
			};
		}
