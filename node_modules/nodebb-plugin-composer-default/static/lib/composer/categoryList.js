...
define('composer/categoryList', function() {
	...
	categoryList.init = function(postContainer, postData) {
		...
		socket.emit('categories.getChildrenAndCategoriesAndByPrivilege', {privilege:'topics:create', parentCids:postData.parentCid}, function(err, categories) {
			if (err) {
			...
			categories.forEach(function(category) {
				if(category.cid == postData.cid){
					postData.parent = category.parentCid
				}
				if (category.parent) {
	...
	function recursive(category, listEl, level) {
		...
		//只有技术论坛和项目社区的子板块显示
		if (category.tpl == '3' || category.cid == '1' || category.cid == '4' || category.cid == '57' || category.cid == '59' || category.cid == '60'){
			var bullet = level ? '&bull; ' : '';
			$('<option value="' + category.cid + '" ' + (category.noPrivilege ? 'disabled' : '') + '>' + level + bullet + category.name + '</option>').appendTo(listEl);
